...
export default function REC27({ initialMonitoring = null }) {
    ...
    useEffect(() => {
        if (initialMonitoring) {
            setData({
                monitoring: {
                    id: initialMonitoring.id,
                    lastReviewDate: initialMonitoring.last_review_date ? initialMonitoring.last_review_date.split('T')[0] : "",
                    defaultRenewalPeriod: initialMonitoring.default_renewal_period || 6,
                    unit: initialMonitoring.unit || "Months",
                    verifiedBy: initialMonitoring.verified_by || "",
                    withDate: initialMonitoring.with_date ? initialMonitoring.with_date.split('T')[0] : "",
                    status: initialMonitoring.status || "Draft",
                },
                items: initialMonitoring.items ? initialMonitoring.items.map(item => ({
                    id: item.id,
                    ...
                    result: item.result || '',
                    limit: item.limit || '',
                    actionRequired: item.action_required || 'No',
                    notes: item.notes || '',
    ...
    const updateAllReviewDates = () => {
        ...
        let period = parseInt(data.monitoring.defaultRenewalPeriod) || 0;
        let nextDate = new Date(today);
        
        if (data.monitoring.unit === 'Months') {
            nextDate.setMonth(nextDate.getMonth() + period);
        } else if (data.monitoring.unit === 'Years') {
    ...
    const handleSave = () => {
        post(route("administrator.rec-forms.rec-27.store"), {
            onSuccess: () => {
        ...
        <AdministratorLayout>
            ...
            <div className="max-w-[1500px] mx-auto space-y-6 pb-12 font-sans text-[#1e293b]">
                <div className="bg-white border-[1px] border-[#e2e8f0] rounded-[8px] p-4 shadow-sm">
                    ...
                    <p className="text-[14px] text-[#64748b]">QH&S hazard identification</p>
                </div>
                    ...
                    <div className="bg-white border-[1px] border-[#0ea5e9] rounded-[10px] p-4 flex gap-4 items-start shadow-sm">
                        ...
                        <div className="flex-1">
                            <h3 className="text-[#0ea5e9] font-bold text-[16px] leading-tight flex items-center gap-1">
                                Monitoring Due Soon! <span className="font-normal text-[15px] ml-1">{overdueItems.length} review(s) due or overdue:</span>
                            </h3>
                            <ul className="mt-1 ml-6 list-disc text-[#0ea5e9] text-[15px] font-medium">
                                {overdueItems.map((item, idx) => (
                                    <li key={idx}>{item.parameter || 'New Parameter'} - <span className="font-bold uppercase">OVERDUE by {Math.abs(calculateDaysRemaining(item.nextReviewDue))} days ({item.nextReviewDue})</span></li>
                                ))}
                            </ul>
                        </div>
                    </div>
                ...
                <div className="border-[1px] border-[#0ea5e9] rounded-[10px] overflow-hidden bg-white shadow-sm">
                    ...
                    <div className="p-4 flex flex-col md:flex-row gap-6 items-end">
                        <div className="flex-1 min-w-[150px]">
                            ...
                            <input
                                type="number"
                                value={data.monitoring.defaultRenewalPeriod}
                                onChange={(e) => setData("monitoring", { ...data.monitoring, defaultRenewalPeriod: e.target.value })}
                        ...
                        <div className="flex-1 min-w-[150px]">
                            ...
                            <select
                                value={data.monitoring.unit}
                                onChange={(e) => setData("monitoring", { ...data.monitoring, unit: e.target.value })}
                                className="w-full h-[45px] bg-[#f8fafc] border-[1px] border-[#cbd5e1] rounded-[6px] px-3 focus:outline-none focus:border-[#0ea5e9]"
                            >
                        ...
                        <div className="flex-1 min-w-[150px]">
                            ...
                            <input
                                type="date"
                                value={data.monitoring.lastReviewDate}
                                onChange={(e) => setData("monitoring", { ...data.monitoring, lastReviewDate: e.target.value })}
                                className="w-full h-[45px] bg-[#f8fafc] border-[1px] border-[#cbd5e1] rounded-[6px] px-3 focus:outline-none focus:border-[#0ea5e9]"
                            />
                        </div>
                        ...
                        <table className="w-full min-w-[1400px] border-collapse bg-white">
                            <thead>
                                <tr className="bg-[#cbd5e1]/40 border-b-[1px] border-[#94a3b8]/50">
                                    <th className="px-3 py-4 text-left text-[14px] font-bold text-[#475569] border-r-[1px] border-[#94a3b8]/40">Parameter</th>
                                    <th className="px-3 py-4 text-left text-[14px] font-bold text-[#475569] border-r-[1px] border-[#94a3b8]/40">Unit</th>
                                    <th className="px-3 py-4 text-left text-[14px] font-bold text-[#475569] border-r-[1px] border-[#94a3b8]/40">Method</th>
                                    <th className="px-3 py-4 text-left text-[14px] font-bold text-[#475569] border-r-[1px] border-[#94a3b8]/40">Result</th>
                                    <th className="px-3 py-4 text-left text-[14px] font-bold text-[#475569] border-r-[1px] border-[#94a3b8]/40">Limit</th>
                                    ...
                                    <th className="px-3 py-4 text-left text-[14px] font-bold text-[#475569] border-r-[1px] border-[#94a3b8]/40">Frequency</th>
                ...
                <div className="bg-[#fef9c3]/50 border-[1px] border-[#fde68a] rounded-[10px] p-6 flex gap-6 items-center shadow-sm">
                    ...
                    <div className="flex-1 space-y-3">
                        <h3 className="text-[#1e2b3c] font-bold text-[20px]">Monitoring Due Alerts</h3>
